...
switch (_group getVariable "SAA_routeType") do {
	case 0 : {
		...
		],[4,6,8],15] call FUNC(addWaypoint);
		...
		switch (_group getVariable "SAA_routeStyle") do {
			case 0 : { // Clockwise Circle
				...
				for "_i" from 0 to 6 do {
					...
					_dir = _dir + 60;
					if (_i != 6) then {
						[_group,_position,0,"MOVE","SAFE","GREEN","LIMITED",_formation,["true",""],[4,6,8],10] call FUNC(addWaypoint);
						if (_i isEqualTo 0) then {
							_group call FUNC(theNudge);
						};
					} else {
						[_group,_position,0,"CYCLE","SAFE","GREEN","LIMITED",_formation,["true",""],[4,6,8],10] call FUNC(addWaypoint);
					};
				};
			};

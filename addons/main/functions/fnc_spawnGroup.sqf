...
private _types = switch (true) do {
	...
	case (_chars isEqualType configNull) : {
		...
		for "_i" from 0 to ((count _chars) - 1) do {
			private _item = _chars select _i;

			if (isClass _item) then {
				_types pushBack getText (_item >> "vehicle");
				_ranks pushBack getText (_item >> "rank");
...
//Create the units according to the selected types.
{
	...
	private _type = _x;
	private _isMan = getNumber (configFile >> "CfgVehicles" >> _type >> "isMan") isEqualTo 1;
...
[{{_x allowDamage true} forEach _this},_spawns,8] call CBA_fnc_waitAndExecute;

...
uiNamespace setVariable ["SAA_CDS_titleText",_titleText];
uiNamespace setVariable ["SAA_CDS_onOK",_onOK];
uiNamespace setVariable ["SAA_CDS_onCancel",_onCancel];
uiNamespace setVariable ["SAA_CDS_customArguments",_customArguments];
createDialog "SAA_CDS_Dialog";
...
private _posY = CENTER_Y - _neededHeight / 2 + BUFFER_H / 1.5;
{
	...
	// Create Description
	private _text = _display ctrlCreate ["SAA_CDS_Text",TEXT_IDC_START + _forEachIndex];
	...
	if (!_forceDefault) then {
		_values = [toUpper _type,_description,_values,false] call SAA_CDS_fnc_cacheValue;
	};
	...
	switch (toUpper _type) do {
		case "CHECKBOX" : {
			...
			private _ctrl = _display ctrlCreate ["SAA_CDS_Checkbox",CTRL_IDC_START + _forEachIndex];
			_ctrl ctrlSetPosition [CTRL_X,_posY + CHECKBOX_Y];
			_ctrl ctrlCommit 0;
			_ctrl cbSetChecked _bool;
			_ctrl setVariable ["SAA_CDS_ctrlInfo",[_type,_description,_bool,_onValueChanged,_enableCondition]];
			_ctrl setVariable ["SAA_CDS_descriptionIDC",TEXT_IDC_START + _forEachIndex];
			...
			_ctrl ctrlAddEventHandler ["CheckedChanged",{
				...
				[_currentValue,uiNamespace getVariable "SAA_CDS_customArguments",_ctrl] call (_ctrlInfo # 3);
			}];
		};
		case "EDITBOX" : {
			_values params [["_string","",[""]]];

			private _ctrl = _display ctrlCreate ["SAA_CDS_Editbox",CTRL_IDC_START + _forEachIndex];
			_ctrl ctrlSetPosition [CTRL_X,_posY + EDITBOX_Y];
			_ctrl ctrlCommit 0;
			_ctrl ctrlSetText _string;
			_ctrl setVariable ["SAA_CDS_ctrlInfo",[_type,_description,_string,_onValueChanged,_enableCondition]];
			_ctrl setVariable ["SAA_CDS_descriptionIDC",TEXT_IDC_START + _forEachIndex];
		...
		case "SLIDER" : {
			...
			private _ctrl = _display ctrlCreate ["SAA_CDS_Slider",CTRL_IDC_START + _forEachIndex];
			...
			_ctrl setVariable ["SAA_CDS_ctrlInfo",[_type,_description,_values,_onValueChanged,_enableCondition]];
			_ctrl setVariable ["SAA_CDS_descriptionIDC",TEXT_IDC_START + _forEachIndex];

			private _aux = _display ctrlCreate ["SAA_CDS_SliderAux",CTRL_AUX_IDC_START + _forEachIndex];
			_aux ctrlSetPosition [SLIDER_AUX_X,_posY + SLIDER_AUX_Y];
			_aux ctrlCommit 0;
			_aux ctrlSetText _fixedValue;
			_aux setVariable ["SAA_CDS_parentIDC",CTRL_IDC_START + _forEachIndex];
			_ctrl setVariable ["SAA_CDS_auxIDC",CTRL_AUX_IDC_START + _forEachIndex];
			...
			_ctrl ctrlAddEventHandler ["SliderPosChanged",{
				...
				private _aux = findDisplay DISPLAY_IDD displayCtrl (_ctrl getVariable "SAA_CDS_auxIDC");
			...
			_aux ctrlAddEventHandler ["KeyUp",{
				params ["_aux"];
				private _ctrl = findDisplay DISPLAY_IDD displayCtrl (_aux getVariable "SAA_CDS_parentIDC");
				private _ctrlInfo = _ctrl getVariable "SAA_CDS_ctrlInfo";
				((_ctrlInfo # 2) # 0) params ["_min","_max","_decimals"];
		...
		case "COMBOBOX" : {
			...
			private _ctrl = _display ctrlCreate ["SAA_CDS_Combobox",CTRL_IDC_START + _forEachIndex];
...
uiNamespace setVariable ["SAA_CDS_controls",_IDCs];
ctrlSetFocus _OK;
...
SAA_CDS_EFID = addMissionEventHandler ["EachFrame",{
	disableSerialization;
	private _display = findDisplay DISPLAY_IDD;

	{
		...
		private _enableCtrl = [_currentValue,uiNamespace getVariable "SAA_CDS_customArguments",_ctrl] call (_ctrlInfo # 4);

		if (!_enableCtrl && ctrlEnabled _ctrl) then {
			_ctrl ctrlEnable false;
			_display displayCtrl (_ctrl getVariable "SAA_CDS_descriptionIDC") ctrlSetTextColor [COLOR_DISABLED];
			if (_ctrlInfo # 0 == "SLIDER") then {ctrlEnable [_ctrl getVariable "SAA_CDS_auxIDC",false];};
		};
		if (_enableCtrl && !ctrlEnabled _ctrl) then {
			_ctrl ctrlEnable true;
			_display displayCtrl (_ctrl getVariable "SAA_CDS_descriptionIDC") ctrlSetTextColor [1,1,1,1];
			if (_ctrlInfo # 0 == "SLIDER") then {ctrlEnable [_ctrl getVariable "SAA_CDS_auxIDC",true];};
		};

		false
	} count (uiNamespace getVariable "SAA_CDS_controls");
}];

// Handle ESC key
_display displayAddEventHandler ["KeyDown",{
	if (_this # 1 == DIK_ESCAPE) then {call SAA_CDS_fnc_cancel;};
	false
}];
